...
import { getAuthSecurity, getCurrentUser, getMyPoints } from "./api";
...
export function useAuthSecurityQuery(enabled = true) {
  const { clearSession, isReady, token } = useAuthSession();
  const query = useQuery({
    queryKey: authQueryKeys.security(),
    queryFn: getAuthSecurity,
...
export function useMyPointsQuery(enabled = true) {
  const { clearSession, isReady, token } = useAuthSession();
  const query = useQuery({
    queryKey: authQueryKeys.points(),
    queryFn: getMyPoints,
    enabled: enabled && isReady && Boolean(token),
    retry: false,
    staleTime: 60_000,
