...
- 已登录：直接进入全站最新流或社区内容。
- 可以使用紧凑 editorial masthead，但不使用全屏 hero。
- 不使用大面积插画或彩虹背景。
...
- 内容区优先显示真实数据。
- 首页可使用更强的栏目标题和编号标签；普通内页标题不超过一行，副信息保持克制。
- 帖子流优先使用面层列表、编号、元信息和数据块建立层级，不默认使用大卡片堆叠，也不靠密集分割线制造结构。
- 顶部和右侧栏的次级入口优先使用文字 bar / 色块动作，避免堆叠 outline button。
...
- 不在没有筛选需求时放复杂筛选栏。
- 不让 hover 改变布局尺寸。
- 不默认用圆角 Card 承载所有列表项；优先考虑面层列表、留白和低对比边界。
...
- 详情页必须区分全局导航和返回来路；全局导航由 App Shell 负责，详情页返回入口只负责回到用户的合理上一步。
- 从首页信息流进入帖子详情时，返回入口应回到信息流或标注为 `返回信息流`。
- 从社区详情进入帖子详情时，返回入口应回到对应社区，例如 `返回 /public`。
- 从搜索结果进入帖子详情时，返回入口应回到当前搜索结果，并保留 query 和 scope。
- 如果无法判断来源，才使用稳定层级 fallback，例如 `返回信息流` 和社区上下文入口；不要硬编码成无关的 `返回社区索引`。
- 右侧栏可以提供社区上下文入口，但不能替代返回来路。
