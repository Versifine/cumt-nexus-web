...
- 不实现编辑 / 预览双模式。
- 不新增后端未支持的结构化 `embed.id` 持久化；canonical 白名单裸链接可以由前端受控播放器渲染。
...
- 列表页主预览舞台不得优先加载原图或小缩略图；优先使用 `medium_url`，没有时回退 `url`，避免把缩略图拉糊。缩略导航和小格子才优先使用 `thumbnail_url`。

详情页媒体规则：

- 普通图按内容列宽完整显示。
- 长图默认限高 `80vh` 并截断预览；点击“展开长图”后按内容列固定宽度完整展开，避免默认把详情页拖成几十屏。
...
- 媒体块分组、列表主预览图、缩略导航、详情中图、原图和可播放 embed 的结构化读取需求记录在根目录 `backend-api-needs.md`。
...
- `/settings/profile` 已作为个人主页编辑入口接入。当前后端尚未提供个人资料更新接口时，页面只能展示读取到的公开资料、字段校验和禁用保存状态；保存合同继续记录在根目录 `backend-api-needs.md`。

非目标：

- 不实现个人资料真实保存，直到后端提供 Bearer 保护的更新接口。
- 不实现关注用户、私信或粉丝列表。
