...
- 写作器：发帖、评论、回复、帖子编辑和评论编辑都使用单一 `MarkdownComposerField` 和格式工具条；组件默认展示发布效果，正文编辑只在用户点击“开始写作”或打开“编辑正文”后显示。
...
2. 白名单 embed 的前端 canonical 裸链接播放器已落地；继续设计后端 resolve / 短链 / 元数据 / `embed_ids` 合同，以及普通网页链接预览、评论投票和通知事件源增强。
