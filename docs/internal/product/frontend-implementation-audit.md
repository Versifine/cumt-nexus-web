...
- 帖子编辑弹窗：已显示统一工具栏和保留原始 Markdown 的正文 textarea，但浏览器保存失败。
- 浏览器保存失败原因：shell 直连 `PATCH /api/v1/posts/:id` 成功；真实 `OPTIONS` 预检返回 `Access-Control-Allow-Methods: GET, POST, PUT, DELETE, OPTIONS`，没有 `PATCH`，所以浏览器 `PATCH` 会被 CORS 拦截。
